...
*Figure 1. One example trace, not percentile measurements. Time to first output and completion expose different parts of the user experience.*
...
*Figure 2. A worked estimate with an assumed benchmark. The same input mix and latency constraint must hold before the replica count has practical meaning.*
...
*Figure 3. Queue separation must be accompanied by limits at shared bottlenecks. Otherwise the workloads meet again at the same saturated resource.*
...
## Release a system version and evaluate the outcome

A deployment can change code, prompt templates, retrieval settings, index generation, model routing, and tool contracts. I would record their identifiers together for each run and expose them in comparison dashboards. Otherwise a quality regression gets attributed to the most visible change while a simultaneous index migration goes unnoticed.

Offline evaluation should cover representative tasks, permission boundaries, incomplete evidence, tool validation, and integration failures. A staged release then compares task outcomes, latency, cost, and failure modes on a limited population. Shadow execution is useful for reads and proposals; production writes must be disabled or redirected to a sandbox to avoid duplicating business actions.

Rollback needs compatibility with outstanding runs and data state. Reverting the application does not undo a refund, and pointing retrieval at an old index does not make its contents current. Each change should state its rollback mechanism and the conditions under which that mechanism remains safe.

The cost metric I care about is total cost divided by successfully completed tasks, with an explicit definition of success for each task class. The numerator includes failed attempts, retries, retrieval, background processing, and allocated infrastructure costs. User abandonment and unresolved cases should remain visible rather than vanish from a flattering average. Human handling effort is also worth measuring where it can be attributed reliably.
