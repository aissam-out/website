...
*Figure 1. The runtime owns progress and recovery. The model proposes work inside that runtime; it does not own the durable execution record.*
...
*Figure 2. Unknown external outcomes have their own state. Treating them as ordinary failures would encourage unsafe retries.*
...
*Figure 3. A timeout changes what the platform knows. Reconciliation establishes the external outcome before the run claims completion.*
