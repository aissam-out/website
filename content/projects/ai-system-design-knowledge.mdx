...
*Figure 1. Discovery and publication have different checkpoints. An observed change remains recoverable while downstream processing catches up.*
...
*Figure 2. Staging isolates incomplete work. The catalog selects the current revision; query-time validation prevents stale candidates from reaching context.*
...
*Figure 3. Candidate generation is scoped, and evidence is checked again before model access. Permissions apply to every path by which text enters the answer.*
